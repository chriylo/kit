 ...
 *
 */
public class Trie4 implements Trie{
	...
	public int addWord(CharSequence word, int start, int end) {
		...
		return addWord(root, end-start, NO_SOURCE);
	}
	
	public int addWord(CharSequence word, int start, int end, int source) {
		setTmpLabel(word, start, end);
		return addWord(root, end-start, source);
	}
	...
	public int addWord(CharSequence word) {
		return addWord(word, 0, word.length(), NO_SOURCE);
	}
	
	public int addWord(CharSequence word, int source) {
		return addWord(word, 0, word.length(), source);
	}
	...
	protected int addWord(State state, int wordLength, int source) {
		if (state.wordLength == wordLength){
			...
			if (source != NO_SOURCE) { if (!state.sources.contains(source)) { state.sources.add(source); } }
			return state.wordIx;
		}
		else{
			...
			if (state.transitions[transitionIx] == null){
				...
				if (source != NO_SOURCE) { if (!state.transitions[transitionIx].sources.contains(source)) { state.transitions[transitionIx].sources.add(source); } }
				++numOfWords; 
				return numOfWords-1;
			}
			else{
				...
				if (lcp == label.length){
					...
					return addWord(state.transitions[transitionIx], wordLength, source);
				}
				else{
					...
					return addWord(newChild, wordLength, source);
				}
			}
		}
	}
	...
	public ArrayList<Integer> getSource(CharSequence word) {
		...
		return getSource(root, word.length());
	}

	protected ArrayList<Integer> getSource(State state, int wordLength) {
		if (state.wordLength == wordLength){
			return state.sources;
		}
		else{
			byte transitionIx = tmpLabel[state.wordLength];
			if (transitionIx == NO_TRANSITION) return new ArrayList<Integer>();		
			...
			if (childState == null || 
				...
				return getSource(childState, wordLength);
			}
		}
	}
	...
	private void forEachDirected(CharSequence word, int start, int end, IntFunction f){
		...
		while (start < end){
			...
			if (shiftBox[0] == 0 && currState.wordIx != NO_WORD){
				f.execute(currState.wordIx, start - prevWordLength, currState.sources);
			}
	...
	public State[] states(){
		State[] states = new State[numOfWords];
	...
	protected class State{
		...
		protected ArrayList<Integer> sources; //this extra bookkeeping is to keep track of which genes the word comes from.
		...
		public void setStates(State[] words) {
			if (wordIx == NO_WORD){
				for (byte transition = (byte) (alphabet.size()-1); transition >= 0; --transition){
					if (transitions[transition] != null){
						transitions[transition].setStates(words);
					}
				}
			}
			else{
